...
**注释只写「这段代码做什么」，不写「为什么这么写」。**

- 只给关键代码添加注释，能通过名称判断的，简单的代码不要添加注释。
- 函数、方法、常量、类型：注释说明其职责，以及必要时的参数与返回值；不需要注释的简单代码不写注释。
- 行内注释只写执行顺序、调用时序等直接的功能说明。
- 禁止把设计理由、取值依据、历史原因、与旧版本的对比、问题背景写进注释。这类内容属于提交信息 或 issue，不属于代码。
- 禁止写「本可以怎样、但选择不怎样」式的权衡说明。
