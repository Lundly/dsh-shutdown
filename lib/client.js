...
`;
		...
		*/
		...
		const SETTINGS_ENTRY_ID = "dsh-shutdown";
		let form;
		/** 由 client/index.ts 在 apply 时绑定条目表单；此后偏好读写均经由该表单。 */
		function bindConfirmForm(bound) {
			form = bound;
		}
		...
		function setConfirmSkipped(skipped) {
			if (!form) return;
			(skipped ? form.set("skipConfirm", true) : form.unset("skipConfirm")).catch(() => {});
		}
		...
		*/
		...
		function ShutdownAction(props) {
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ErrorBoundary, { children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ActionInner, { ...props }) });
		}
		//#endregion
		...
		*/
		...
		function apply(ctx) {
			injectStyles();
			bindConfirmForm(ctx.configForms.get(SETTINGS_ENTRY_ID));
			ctx.effect(() => {
